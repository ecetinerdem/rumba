...
func Astar(room *Room, start, goal Point) []Point {
	...
	for pq.Len() > 0 {
		// Get the point with lowest cost from priority queue

		currentItem := heap.Pop(&pq).(*PQItem)
		current := currentItem.point
		delete(openSetItems, current)

		// If we reached to goal, reconstruct and return the path
		...
		for _, direction := range directions {
			neighbour := Point{X: current.X + direction[0], Y: current.Y + direction[1]}
			// Skip the invalid neighnour or in the closed set
			if !room.IsValid(neighbour.X, neighbour.Y) || closedSet[neighbour] {
				continue
			}
			// Calculate tentative g-score
			tentativeGScore := gScore[current] + 1

			if _, exist := gScore[neighbour]; !exist || tentativeGScore < gScore[neighbour] {
				...
				if item, exist := openSetItems[neighbour]; exist {
					pq.Update(item, fScore[neighbour])
				} else {
					// Add new point to the priority queue
					neighbourItem := &PQItem{
						point:    neighbour,
						priority: fScore[neighbour],
					}

					heap.Push(&pq, neighbourItem)
					openSetItems[neighbour] = neighbourItem
				}
			}
